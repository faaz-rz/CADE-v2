...
import { ContractService, ContractRenewal, RenewalsResponse } from '../services/api';
...
export const ContractCalendar: React.FC = () => {
    ...
    if (!data || data.total_renewals_90_days === 0) {
        return (
            <div className="max-w-5xl mx-auto px-4 py-8">
            ...
            <div className="mt-8 bg-white rounded-xl border border-gray-200 px-5 py-4 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">
                        <span className="font-semibold">{data.total_renewals_90_days} renewal{data.total_renewals_90_days !== 1 ? 's' : ''}</span> in the next 90 days
                    </span>
                </div>
                <span className="text-sm font-bold text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-200">
                    {formatCurrency(data.total_savings_opportunity)} savings opportunity
                </span>
            </div>
        </div>
    );
